...
int main( int argc, char **argv )
{
    ...
    rc = pthread_create( &readStdinThreadId, NULL, ReadStdinThread, argv[2] );
    if ( rc != 0 )
    {
...
void *ReadSerialThread( void *param )
{
	...
	/*while ( 1 )
    {
        ...
        if ( gDebug )
        {
            ...
            }//end of change put into a text file
        }
...
void *ReadStdinThread( void *param ) 
{
	...
	while(!feof(stream)){
		...
		while(1){
			if (count == modem1.LOMsg || feof(stream)){
				break;
			}
			ch = fgetc(stream);
			if(ch == '\r' || ch == '\n' || ch == EOF){
		...
		get_timestamp(tmp);
		snprintf(fixed_pattern_line, sizeof(fixed_pattern_line), "<%s/%s>_%s_", packet_counter_str, modem1.fixed_NoMc, tmp);
		...
		snprintf(Cmd_send, sizeof(Cmd_send), "%s%s", modem1.target_command, HEX);
		...
		if ( gDebug )
        {
				...
				if ( write( gPortFd, &Cmd_send, strlen(Cmd_send) ) != strlen(Cmd_send) )// change to calculate the size of the command plus the data
				{
