...
int main( int argc, char **argv )
{
	...
	date = get_one_time_date();
	fprintf(Files.outputFile, "%s  %s %s %s %s %s\n" ,"The arguments given to execute this program are:", argv[3], argv[4], argv[5], argv[6], argv[7]);
	fprintf(Files.outputFile, "%s  %s\n" ,"The current date and time of this trial is:", date);
    fprintf(Files.outputFile, "%s		%s			%s		%s\n" ,"Send/Receive", "Time Stamp", "Category", "Data");
...
void *ReadSerialThread( void *param )
{
	...
	if(ch == '\n' || ch == '\r'){
		...
		if ( gDebug )
    		{
				if(strncmp(line, ERR, strlen(ERR)) == 0){
					fprintf(Files.outputFile, ">>			%s			%s			%s\n", tmp, "ERR" ,line);
				}
				if(strncmp(line, OKY, strlen(OKY)) == 0){
					fprintf(Files.outputFile, ">>			%s			%s			%s\n", tmp, "MDM" ,line);
				}
				if((strncmp(line, RECX, strlen(RECX)) == 0) || (strncmp(line, RECS, strlen(RECS)) == 0)){
					fprintf(Files.outputFile, ">>			%s			%s			%s\n", tmp, "REC" ,line);
				}
          		//fprintf(Files.outputFile, ">>			%s			%s\n", tmp ,line);  
    		}
...
void *ReadStdinThread( void *param )
{
	...
	if(modem1.NBMsg == 0){
		printf("Nothing to read-->exiting the reading thread....\n");
		pthread_exit(NULL);
	}
	char ch;
	char	nl = '\r';
	...
	while(!feof(Files.inputFile2) && packet_counter <= (modem1.NBMsg - 1)){
	   ...
	   if ((packet_counter < modem1.NBMsg) && (!feof(Files.inputFile2))){
